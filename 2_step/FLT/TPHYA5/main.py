...
class Transition:
    # предыдущее состояние, входящий символ, следующий шаг
...
class Automate:
    ...
    def ways(self, node: str) -> List[Transition]:
        # ВОЗВРАЩАЕТ ЛИСТ VALUES КОТОРЫЕ ПЕРЕХОДЯТ ИЗ ПРЕДЫДУЩЕГО В ВЫБРАННОЕ СОСТОЯНИЕ
    ...
    def check_all_ways(self, nodes: str) -> Dict[str, str]:
        ...
        for node, node_w in overall_ways.items():
            ...
            # ПЕРЕХОД К СЛЕДУЮЩЕМУ НОДУ ЕСЛИ ТЕКУЩИЙ ПУСТОЙ
